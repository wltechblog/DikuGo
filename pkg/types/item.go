...
// CanClassUseItem checks if a character class can use a specific item
func CanClassUseItem(class int, obj *Object) bool {
	// Check class-specific anti-flags
	switch class {
	case CLASS_MAGIC_USER:
		if obj.ExtraFlags&ITEM_ANTI_MAGIC != 0 {
			return false
		}
	case CLASS_CLERIC:
		if obj.ExtraFlags&ITEM_ANTI_CLERIC != 0 {
			return false
		}
	case CLASS_THIEF:
		if obj.ExtraFlags&ITEM_ANTI_THIEF != 0 {
			return false
		}
	case CLASS_WARRIOR:
		if obj.ExtraFlags&ITEM_ANTI_WARRIOR != 0 {
			return false
		}
	}
	...
	switch obj.Type {
	case ITEM_ARMOR:
		// Magic users can only wear light armor (AC bonus <= 2)
		if class == CLASS_MAGIC_USER && obj.Value[0] > 2 {
			return false
		}
		// Thieves can only wear medium armor (AC bonus <= 4)
		if class == CLASS_THIEF && obj.Value[0] > 4 {
			return false
		}
	case ITEM_WEAPON:
		// Magic users can only use daggers, staves, and wands
		if class == CLASS_MAGIC_USER {
			weaponType := obj.Value[0]
